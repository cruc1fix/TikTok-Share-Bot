...
class TikTok:
    def __init__(self):
        ...
        print("          Share Bot V4  ")
        print("          Share bot for tiktok  ")
        print("! IT MIGHT TAKE UP TO 24 HOURS TO SHOW !")
        print("           Newest generation")
        ...
        try:
            self.amount = int(input('> Input Amount Of Share Here: '))
        except ValueError:
    ...
    def close(self, message):
        print(f'\n{message}')
        os.system("title crucifix's TikTok SharesBot - Restart required")
        os.system('pause >NUL')
        os.system("title crucifix's TikTok SharesBot - Closing...")
        sleep(3)
        os._exit(0)
    ...
    def update_title(self):
        ...
        while self.added < self.amount:
            ...
            os.system(
                f"title Scripting's TikTok SharesBot - Added: {self.added} Shares/{self.amount} Shares "
                f'({round(((self.added / self.amount) * 100), 3)}%) ^| Active Threads: '
                f'{threading.active_count()} ^| Time Remaining: {time_remaining}'
            )
            sleep(0.2)
        os.system(
            f"title Scripting's TikTok SharesBot - Added: {self.added}/{self.amount} "
            f'({round(((self.added / self.amount) * 100), 3)}%) ^| Active Threads: '
...
if __name__ == '__main__':
    os.system("cls && title crucifix's TikTok SharesBot")
    main = TikTok()
    main.start()
